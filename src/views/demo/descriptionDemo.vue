<template>
  <div>
    ...
    <t-description direction="vertical" :descData="data" border size="small" :column="4" title="详情框">
      ...
      <template #goodsSlot="scope"> <i class="el-icon-user"></i>{{ scope.label }} </template>
      <template #total>
        <span :style="{ color: 'red' }">总件数</span>
      </template>
      <!-- 模块detail详情content内容插槽,也可通过作用域插槽获取value -->
      <template #pic="scope">
        <img :src="scope.value" width="20" />
      </template>
    </t-description>
  </div>
</template>
