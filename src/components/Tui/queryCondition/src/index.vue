<template>
  <el-form
    :label-width="labelWidth"
    :model="form"
    class="t-query-condition"
    :style="{ 'grid-template-areas': gridAreas, 'grid-template-columns': `repeat(${colLength}, minmax(220px, ${100 / colLength}%))` }"
  >
    <el-form-item v-for="(opt, i) in cOpts" :key="i" :label="opt.label" :style="{ gridArea: i }">
      <OptComponent v-bind="opt" :opt="opt" :value="form[opt.dataIndex]" @change="change" />
    </el-form-item>
    <el-form-item v-if="Object.keys(cOpts).length > 0" label-width="0" style="grid-area: submit_btn" :class="{ flex_end: cellLength % colLength === 0 }">
      <el-button type="primary" @click="checkHandle" :loading="loading"
        ><el-icon><search /></el-icon>查询</el-button
      >
      <el-button v-if="reset" @click="resetHandle"
        ><el-icon><refresh /></el-icon>重置</el-button
      >
      <el-button v-if="add" type="primary" @click="addHandle"
        ><el-icon><plus /></el-icon>新增</el-button
      >
    </el-form-item>
  </el-form>
</template>

<script lang="ts">
import OptComponent from './optComponent.vue';
import { computed, defineComponent, onMounted, reactive, ref, watchEffect } from 'vue';
export default defineComponent({
  name: 'TQueryCondition',
  components: {
    OptComponent
  },
  props: {
    // 配置项
    opts: {
      type: Object,
      default: () => ({})
    },
    //label宽度
    labelWidth: {
      type: String,
      default: '100px'
    },
    // loading状态
    loading: {
      type: Boolean,
      default: false
    },
    //重置按钮显示
    reset: {
      type: Boolean,
      default: true
    },
    //新增按钮显示
    add: {
      type: Boolean,
      default: false
    },
    // 是否回车查询
    boolEnter: {
      type: Boolean,
      default: true
    }
  },
  setup(props, { emit }) {
    const opts = reactive(props.opts);
    let colLength = ref(0);
    const formArr = Object.keys(opts).reduce((acc, field) => {
      acc[field] = opts[field].defaultVal || null;
      return acc;
    }, {});
    let form = reactive(formArr);
    let cOpts = computed(() => {
      return Object.keys(opts).reduce((acc, field) => {
        let opt = {
          ...opts[field]
        };
        opt.dataIndex = field;
        acc[field] = opt;
        return acc;
      }, {});
    });
    // grid布局按钮位置
    let gridAreas = computed(() => {
      let template = '\'. . . .\'';
      switch (colLength.value) {
        case 3:
          template = '\'. . .\'';
          break;
        case 2:
          template = '\'. .\'';
          break;
      }
      let areas = [template];
      Object.keys(opts).forEach((key) => {
        // 根据控件描述注定占用多少列及顺序
        let span = 1;
        if (opts[key].span > 1 || opts[key].span <= 2) {
          // 最多占用2列
          span = opts[key].span;
        }
        // 计算剩余多少未占用的位置
        let count = 0;
        let scrstr = areas[areas.length - 1];
        while (scrstr.indexOf('.') !== -1) {
          scrstr = scrstr.replace(/\./, '');
          count++;
        }
        // 若剩余位置不足以放下下一个控件
        if (count < span) {
          areas.push(template);
        }
        let i = 0;
        while (i < span) {
          areas[areas.length - 1] = areas[areas.length - 1].replace(/\./, key);
          i++;
        }
      });
      // 若控件正好占满一行时，补充多一列放置btn
      if (areas[areas.length - 1].indexOf('.') === -1) {
        areas.push(template);
      }
      if (cellLength.value % colLength.value === 0) {
        // 正好占满一行
        areas[areas.length - 1] = areas[areas.length - 1].replace(/\.'$/, 'submit_btn\'');
      } else {
        areas[areas.length - 1] = areas[areas.length - 1].replace(/\./, 'submit_btn');
      }
      return (areas + '').replace(/,/g, '');
    });
    // 占用单元格长度
    let span = computed(() => {
      let span = 1;
      Object.keys(opts).forEach((key) => {
        span = opts[key].span > 4 ? 4 : opts[key].span || 1;
      });
      return span;
    });
    let cellLength = computed(() => {
      let length = 0;
      Object.keys(props.opts).forEach((key) => {
        let span = opts[key].span > 4 ? 4 : opts[key].span || 1;
        length += span;
      });
      return length;
    });
    // 行列数
    const getColLength = () => {
      const width = window.innerWidth;
      let colLength = 4;
      if (width > 768 && width < 1280) {
        colLength = 3;
      } else if (width <= 768) {
        colLength = 2;
      }
      return colLength;
    };
    // 初始化form
    const initForm = (opts: Record<string, any>, keepVal = false) => {
      return Object.keys(opts).reduce((acc, field) => {
        if (keepVal && form) {
          acc[field] = form[field];
        } else if (opts[field].defaultVal) {
          acc[field] = opts[field].defaultVal;
        } else {
          acc[field] = null;
        }
        return acc;
      }, {});
    };
    // 重置操作
    const resetHandle = () => {
      form = initForm(opts);
      checkHandle();
    };
    // 新增操作
    const addHandle = () => {
      emit('add');
    };
    const change = (v, dataIndex: string) => {
      if (dataIndex) {
        form[dataIndex] = v;
        console.log(form);
        emit('change', {
          ...form
        });
      }
    };
    // 查询
    const checkHandle = () => {
      emit('submit', form);
    };
    // 监听配置的变化
    watchEffect(() => {
      form = initForm(opts, true);
    });
    onMounted(() => {
      colLength.value = getColLength();
      if (props.boolEnter) {
        document.onkeydown = function (e) {
          var key = e.keyCode;
          if (key === 13) {
            checkHandle();
          }
        };
      }
    });
    return {
      colLength,
      form,
      cOpts,
      gridAreas,
      span,
      cellLength,
      resetHandle,
      addHandle,
      change,
      checkHandle
    };
  }
});
</script>

<style lang="less">
.t-query-condition.el-form {
  position: relative;
  display: grid;
  gap: 2px 8px;
  margin-bottom: -7px;
  text-align: left;
  .el-select,
  .el-date-editor,
  .ant-calendar-picker {
    width: 100%;
  }
  .flex_end {
    grid-area: submit_btn;
    .el-form-item__content {
      display: flex;
      justify-content: flex-end;
      align-items: center;
    }
  }
  .el-form-item {
    display: flex;
    margin-bottom: 6px;
    .el-form-item__label {
      flex-shrink: 0;
      min-width: 60px;
      padding-left: 8px;
    }
    .el-form-item__content {
      flex-grow: 1;
      overflow: hidden;
      margin-left: 0 !important;
    }
  }
  .btn_check {
    position: relative;
    top: -1px;
  }
  .btn_reset {
    position: relative;
    top: -1px;
    margin-left: 8px;
  }
}
</style>
